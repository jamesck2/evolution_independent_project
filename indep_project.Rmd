...
plants.year.sum <- metadata.long %>% group_by(trait, plantsp, fert, dataset_year) %>% summarize(count = n(),
            mean = mean(value, na.rm = TRUE), 
            SE = sd(value, na.rm = TRUE)/sqrt(count))

strains.year.sum <- metadata.long %>% group_by(trait, strain, fert, dataset_year) %>% summarize(count = n(),
            mean = mean(value, na.rm = TRUE), 
            SE = sd(value, na.rm = TRUE)/sqrt(count))
...
```{r}
## by year
(plot.nodnum.year <- ggplot(data = strains.year.sum %>% filter(trait=="nodnum" & fert=="Fertilized"), aes(x=dataset_year, y=mean, group=strain, color=strain)) +
...
(plot.nodnum.year <- ggplot(data = strains.year.sum %>% filter(trait=="nodnum" & fert=="0 N"), aes(x=dataset_year, y=mean, group=strain, color=strain)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Year") +
   ylab("Nodule number") +
...
```{r}
## by year
(plot.shootmass.year <- ggplot(data = plants.year.sum %>% filter(trait=="nodnum" & fert=="0 N"), aes(x=dataset_year, y=mean, group=plantsp, color=plantsp)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Year") +
   ylab("Nodnum") +
...
lmm_shoot_2008 <- lmer(sqrt(shootmass) ~ plantsp + dataset_year + (plantsp:dataset_year) + (1|harvester) + (1|field) + (1|plot) + (1|strain), data = metadata)
summary(lmm_shoot_2008)
Anova(lmm_shoot_2008, type = 3)

# model without plot as a random effect
lmm_shoot_2008_dplot <- lmer(sqrt(shootmass) ~ plantsp + dataset_year + (plantsp:dataset_year) + (1|harvester) + (1|field) + (1|strain), data = metadata)
...
# 2018
# full model with all random effects
lmm_shoot_2018 <- lmer(sqrt(shootmass) ~ plantsp + dataset_year + (plantsp:dataset_year) +(1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(dataset_year=="2018"))
...
# 2008
# full model with all random effects
lmm_nod_2008 <- lmer(sqrt(nodnum) ~  plantsp + (1|field) + (1|harvester) + (1|plot) + (1|strain), data = metadata %>% filter(dataset_year== "2008"))
Anova(lmm_nod_2008, type = 3)

# model without plot as a random effect
lmm_nod_2008_dplot <- lmer(sqrt(nodnum) ~ plantsp + (1|field) + (1|harvester) + (1|plot) , data = metadata %>% filter(dataset_year=="2008"))
...
#2018
# full model with all random effects
lmm_nod_2018 <- lmer(sqrt(nodnum) ~ plantsp + dataset_year + (plantsp:dataset_year) +(1|harvester) + (1|field) + (1|plot) + (1|strain), data = metadata %>% filter(dataset_year=="2018"))
