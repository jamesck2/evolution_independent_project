...
plants.year.sum <- metadata.long %>% group_by(trait, plantspori, dataset_year) %>% summarize(count = n(),
...
```{r}
## by year
(plot.nodulenum.year <- ggplot(data = strains.year.sum %>% filter(trait=="nodulenum" & nitrogen=="Nitrogen"), aes(x=dataset_year, y=mean, group=strain, color=strain)) +
...
(plot.nodulenum.year <- ggplot(data = strains.year.sum %>% filter(trait=="nodulenum" & nitrogen=="Control"), aes(x=dataset_year, y=mean, group=strain, color=strain)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Year") +
   ylab("Nodule number") +
...
## by environment
(plot.nodulenum.env <- ggplot(data = strains.nitrogen.sum %>% filter(trait=="nodulenum"), aes(x=fert, y=mean, group=strain, color=strain)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Environment") +
   ylab("Nodule number") +
...
## by environment
(plot.nodmass.env <- ggplot(data = strains.nitrogen.sum %>% filter(trait=="meannodulemass"), aes(x=fert, y=mean, group=strain, color=strain)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Environment") +
   ylab("Nodule mass") +
...
## by N environment
(plot.chloro.env <- ggplot(data = plants.nitrogen.sum %>% filter(trait=="chloro"), aes(x=fert, y=mean, group=plantspori, color=plantspori)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Environment") +
   ylab("Chlorophyll content") +
...
```{r}
## by year
(plot.shootmass.year <- ggplot(data = plants.year.sum %>% filter(trait=="nodulenum" & nitrogen=="Control"), aes(x=dataset_year, y=mean, group=plantspori, color=plantspori)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Year") +
   ylab("nodulenum") +
...
## by N environment
(plot.shootmass.env <- ggplot(data = plants.nitrogen.sum %>% filter(trait=="shootmass"), aes(x=fert, y=mean, group=plantspori, color=plantspori)) +
   geom_point(size=3, position = position_dodge(0.3)) +
   geom_line(position = position_dodge(0.3)) +
   xlab("Environment") +
   ylab("Shoot biomass") +
...
lmm_shoot_2008 <- lmer(sqrt(shootmass) ~ plantspori + dataset_year + (plantspori:dataset_year) + (1|harvester) + (1|field) + (1|plot) + (1|strain), data = metadata)
summary(lmm_shoot_2008)
Anova(lmm_shoot_2008, type = 3)

# model without plot as a random effect
lmm_shoot_2008_dplot <- lmer(sqrt(shootmass) ~ plantspori + dataset_year + (plantspori:dataset_year) + (1|harvester) + (1|field) + (1|strain), data = metadata)
...
# 2018
# full model with all random effects
lmm_shoot_2018 <- lmer(sqrt(shootmass) ~ plantspori + dataset_year + (plantspori:dataset_year) +(1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(dataset_year=="2018"))

# model without plot as a random effect
lmm_shoot_2018_dplot <- lmer(sqrt(shootmass) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(dataset_year=="2018"))
...
# 0N
# full model with all random effects
lmm_shoot_0N <- lmer(sqrt(shootmass) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Control"))

# model without plot as a random effect
lmm_shoot_0N_dplot <- lmer(sqrt(shootmass) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Control"))
...
# N-fertilized
# full model with all random effects
lmm_shoot_fertilized <- lmer(sqrt(shootmass) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Nitrogen"))

# model without plot as a random effect
lmm_shoot_fertilized_dplot <- lmer(sqrt(shootmass) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Nitrogen"))
...
# 2008
# full model with all random effects
lmm_chloro_2008 <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(dataset_year=="2008"))

# model without plot as a random effect
lmm_chloro_2008_dplot <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(dataset_year=="2008"))
...
#2018
# full model with all random effects
lmm_chloro_2018 <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(dataset_year=="2018"))

# model without plot as a random effect
lmm_chloro_2018_dplot <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(dataset_year=="2018"))
...
# 0N
# full model with all random effects
lmm_chloro_0N <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Control"))

# model without plot as a random effect
lmm_chloro_0N_dplot <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Control"))
...
# N-fertilized
# full model with all random effects
lmm_chloro_fertilized <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Nitrogen"))

# model without plot as a random effect
lmm_chloro_fertilized_dplot <- lmer(sqrt(chloro) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Nitrogen"))
...
# 2008
# full model with all random effects
lmm_nod_2008 <- lmer(sqrt(nodulenum) ~  plantspori + (1|field) + (1|harvester) + (1|plot) + (1|strain), data = metadata %>% filter(dataset_year== "2008"))
Anova(lmm_nod_2008, type = 3)

# model without plot as a random effect
lmm_nod_2008_dplot <- lmer(sqrt(nodulenum) ~ plantspori + (1|field) + (1|harvester) + (1|plot) , data = metadata %>% filter(dataset_year=="2008"))
Anova(lmm_nod_2008_dplot)
...
#2018
# full model with all random effects
lmm_nod_2018 <- lmer(sqrt(nodulenum) ~ plantspori + dataset_year + (plantspori:dataset_year) +(1|harvester) + (1|field) + (1|plot) + (1|strain), data = metadata %>% filter(dataset_year=="2018"))
summary(lmm_nod_2018)
Anova(lmm_nod_2018, type = 3)

# model without plot as a random effect
lmm_nod_2018_dplot <- lmer(sqrt(nodulenum) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(dataset_year=="2018"))
...
# 0N
# full model with all random effects
lmm_nod_0N <- lmer(sqrt(nodulenum) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Control"))

# model without plot as a random effect
lmm_nod_0N_dplot <- lmer(sqrt(nodulenum) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Control"))
...
# N-fertilized
# full model with all random effects
lmm_nod_fertilized <- lmer(sqrt(nodulenum) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Nitrogen"))

# model without plot as a random effect
lmm_nod_fertilized_dplot <- lmer(sqrt(nodulenum) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Nitrogen"))
...
# 2008
# full model with all random effects
lmm_nodn_2008 <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(dataset_year=="2008"))

# model without plot as a random effect
lmm_nodn_2008_dplot <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(dataset_year=="2008"))
...
#2018
# full model with all random effects
lmm_nodn_2018 <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(dataset_year=="2018"))

# model without plot as a random effect
lmm_nodn_2018_dplot <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(dataset_year=="2018"))
...
# 0N
# full model with all random effects
lmm_nodn_0N <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Control"))

# model without plot as a random effect
lmm_nodn_0N_dplot <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Control"))
...
# N-fertilized
# full model with all random effects
lmm_nodn_fertilized <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field) + (1|plot), data = metadata %>% filter(nitrogen=="Nitrogen"))

# model without plot as a random effect
lmm_nodn_fertilized_dplot <- lmer(sqrt(meannodulemass) ~ plantspori + (1|harvester) + (1|field), data = metadata %>% filter(nitrogen=="Nitrogen"))
